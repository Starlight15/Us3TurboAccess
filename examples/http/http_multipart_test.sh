#!/usr/bin/env bash
# HTTP Multipart 一键端到端测试：启 gateway → 跑 http_multipart_example → 自动清理。
set -u
set -o pipefail
...
EXAMPLE_BIN="${EXAMPLE_BIN:-${US3_BUILD_DIR}/examples/us3_turbo_access_http_multipart_example}"
...
KEY="${KEY:-claude/http-mpu-test-$(date +%s)}"
...
[[ -x "${EXAMPLE_BIN}" ]] || { log "http_multipart_example not found: ${EXAMPLE_BIN}"; exit 2; }
...
log "running http_multipart_example: total=${TOTAL_BYTES} part=${PART_SIZE} concurrency=${CONCURRENCY} bucket=${BUCKET} key=${KEY}"
