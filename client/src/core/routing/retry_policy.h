 ...
 * 三通路共享的重试策略：指数退避 + 抖动。
 *
 *   - max_attempts==1 等价于不重试（CompleteUpload / Abort 等非幂等接口设这个）
 *   - 仅对 error.retryable==true 的失败做重试；其它错误直接透传
 *
 * 算法：第 n 次重试 sleep = min(max_backoff, initial * 2^(n-1)) * jitter，
 *      jitter ∈ [0.5, 1.5)，避免雷鸣群效应。
 *
 * 历史：本类原仅在 HttpDataClient 内使用（client/src/data/http_retry.h），
 * Phase 1.2 提升为公共层，RDMA/GDS 通路也走同一策略。
 */
struct RetryPolicy {
  int                       max_attempts{3};
...
/** 从 HttpClientOptions 构造重试策略（保留旧入口，HTTP 通路用）。 */
...
/**
 * 通用默认重试策略：三通路 PutObject/GetObject/UploadPart 用。
 * 三次尝试 + 100ms 初始退避 + 2s 上限，与 HTTP 默认一致。
 */
[[nodiscard]] inline RetryPolicy DefaultRetryPolicy() {
  return RetryPolicy{};
}
