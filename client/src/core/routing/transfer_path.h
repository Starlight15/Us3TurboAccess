...
class TransferPath {
 public:
  ...
  [[nodiscard]] virtual DataFlow path() const = 0;
  [[nodiscard]] virtual bool available() const = 0;
  [[nodiscard]] virtual Result<TransferOutcome> GetObject(const GetObjectRequest& request,
                                                          MutableBufferView buffer) const = 0;
  [[nodiscard]] virtual Result<TransferOutcome> PutObject(const PutObjectRequest& request,
   ...
   *
   *   path_for_error  : 用于错误码 / 日志；DataFlow::kHttpTcp 等
  ...
  [[nodiscard]] static Result<bool>
    CommonPreflight(DataFlow path_for_error, std::string_view path_name,
