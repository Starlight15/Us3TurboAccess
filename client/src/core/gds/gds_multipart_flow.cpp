...
class GdsMultipartSession final : public MultipartSessionBase {
 public:
  GdsMultipartSession(const MetadataClient& metadata,
      ...
      : MultipartSessionBase(std::move(object), std::move(upload_id),
                             max_part_size, /*set_length=*/false),
        metadata_(metadata),
        transfer_path_(transfer_path) {}

  // ---- 传输层转发 ----
  // length intentionally unset (set_length=false): gateway skips whole-object
  // Reserve on session open.
