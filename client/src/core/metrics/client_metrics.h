 ...
 *      <path> 取自 ToString(DataFlow): "http-tcp" / "native-rdma" / "gds-cuobject"
 *
...
class ClientMetrics {
 public:
  ...
  // PUT / GET / HEAD / UploadPart 共 4 个全局 + 各 2 个 per-path
  OpCounters put;
  OpCounters get;
  OpCounters head;
  ...
  std::array<std::unique_ptr<OpCounters>, 2> put_by_path;          // [GdsCuObject, NativeRdma]
  std::array<std::unique_ptr<OpCounters>, 2> get_by_path;
  std::array<std::unique_ptr<OpCounters>, 2> head_by_path;
  std::array<std::unique_ptr<OpCounters>, 2> upload_part_by_path;
 ...
 * 同时灌 latency 直方图。
 *
 * data_flow 可选：传了就同时记 per-path 拆分（C.4）；不传只记全局。
 */
class ScopedTransferMetric {
 public:
  ...
  void SetDataPath(DataFlow p) noexcept { data_path_ = p; }
 ...
 private:
  ...
  std::optional<DataFlow>  data_path_;
};
